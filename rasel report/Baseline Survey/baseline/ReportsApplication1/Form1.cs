...
namespace ReportsApplication1
{
    public partial class Form1 : Form
    {
        ...
        {
            // TODO: This line of code loads data into the 'DataSet2.Table11' table. You can move, or remove it, as needed.
            this.Table11TableAdapter.Fill(this.DataSet2.Table11);
        ...
        private void set_1()
        {
             ...
             query1 = "SELECT DISTINCT x.a AS union_name, x.b AS male, y.c AS female , isnull(x.b , 0)+ isnull(y.c,0) as total FROM         (SELECT     [union].union_name AS a, COUNT(member.member_id) AS b FROM          [union] INNER JOIN  cnc ON [union].union_id = cnc.union_id INNER JOIN  khana ON cnc.cnc_id = khana.cnc_id INNER JOIN    member ON khana.khana_id = member.khana_id WHERE      (member.sex_no = 1) GROUP BY [union].union_name) x left JOIN  (SELECT     [union].union_name AS a, COUNT(member.member_id) AS c FROM          [union] INNER JOIN cnc ON [union].union_id = cnc.union_id INNER JOIN  khana ON cnc.cnc_id = khana.cnc_id INNER JOIN member ON khana.khana_id = member.khana_id     WHERE      (member.sex_no = 2)  GROUP BY [union].union_name) y ON x.a = y.a"; 
             query2 = "SELECT  [union].union_name FROM         thana INNER JOIN   [union] ON thana.thana_id = [union].thana_id WHERE     (thana.thana_name = '"+comboBox_Thana.Text.ToString()+"') ";
        ...
        private void set_7()
        {
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                ...
                foreach (DataRow r1 in ds.Tables[1].Rows)
                {
                    ...
                    DataColumn column1 = ds.Tables[1].Columns[0];
                    if (row[column].ToString() == r1[column1].ToString())
                    {
                        row[0] = r[0];
                        row[1] = r[1];
                        row[2] = r[2];
                        row[3] = r[3];
                        data_set.Table7.Rows.Add(r);
                    }
                }
            }
